// FileSource.h

#ifndef _UTIL_STREAM_URL_FILE_SOURCE_H_
#define _UTIL_STREAM_URL_FILE_SOURCE_H_

#include "util/stream/UrlSource.h"

#include <framework/filesystem/File.h>

namespace util
{
    namespace stream
    {

        class FileSource
            : public UrlSource
        {
        public:
            FileSource(
                boost::asio::io_service & io_svc);

            virtual ~FileSource();

        public:
            virtual bool open(
                framework::string::Url const & url,
                boost::uint64_t beg, 
                boost::uint64_t end, 
                boost::system::error_code & ec);

            virtual bool open(
                std::string const & path,
                boost::uint64_t beg, 
                boost::uint64_t end, 
                boost::system::error_code & ec);

            virtual bool is_open(
                boost::system::error_code & ec);

            virtual bool close(
                boost::system::error_code & ec);

            virtual boost::uint64_t total(
                boost::system::error_code & ec);

        private:
            // implement util::stream::Source
            virtual std::size_t private_read_some(
                boost::asio::mutable_buffer const & buffer,
                boost::system::error_code & ec);

        private:
            framework::filesystem::File file_;
            bool is_open_;
        };

        UTIL_REGISTER_URL_SOURCE("file", FileSource);

    } // namespace stream
} // namespace util

#endif // _UTIL_STREAM_URL_FILE_SOURCE_H_
